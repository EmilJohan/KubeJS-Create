// The KubeJS version of this component has a bug that renders
// it completely
...
public record EitherRecipeComponent2<H, L>(RecipeComponent<H> high, RecipeComponent<L> low) implements RecipeComponent<Either<H, L>> {
	public static final RecipeComponentFactory FACTORY = RecipeComponentFactory.readTwoComponents(EitherRecipeComponent2::new);
	...
	public void validate(Either<H, L> value) {
		...
		// MOD: the built-in EitherRecipeComponent does not
		// check the optional correctly and unconditionally
		// causes a crash
		if (left.isPresent()) {
